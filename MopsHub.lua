...
local function showOwnerPasswordPrompt(onComplete)
    ...
    local function trySubmit()
        ...
        if checkPassword(passBox.Text) then
            submitted = true isOwner = true myRole = "Owner"
            saveRole("Owner")
...
local function showRoleSelection(onDone)
    ...
    makeRoleBtn(318, "Owner", "Password", ROLE_COLORS.Owner, "OWNER", true, false)
end
...
local function startAutoRewards()
    if autoRewardsRunning then return end
    autoRewardsRunning = true
    autoRewardsThread = task.spawn(function()
        while autoRewardsRunning do
            if not STATE.Maintenance and CONFIG.AutoRewards then
                pcall(function()
                    local n = claimPlaytimeRewards()
...
local function stopAutoRewards()
    autoRewardsRunning = false
    if autoRewardsThread then
        pcall(function() task.cancel(autoRewardsThread) end)
        autoRewardsThread = nil
    end
end
...
local function applyAllFromConfig()
    ...
    if CONFIG.AutoRewards then startAutoRewards() else stopAutoRewards() end
end
...
local BIND_FUNCS = {
    ...
    {id = "autorewards", name = "Auto Rewards", toggle = function() CONFIG.AutoRewards = not CONFIG.AutoRewards if CONFIG.AutoRewards then startAutoRewards() else stopAutoRewards() end end},
}
...
function rebuildContent()
    ...
    if currentTab == "combat" then
        ...
        local c6 = makeCard("Auto Rewards", 6)
        addToggle(c6, 56, "Enable", CONFIG.AutoRewards, function(s) CONFIG.AutoRewards = s if s then startAutoRewards() else stopAutoRewards() end end)
        addSlider(c6, 90, "Delay", 1, 10, CONFIG.RewardsDelay, function(v) CONFIG.RewardsDelay = v end)
